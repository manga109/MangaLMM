...
def default_evaluation_params():
    ...
    global NED  # for NED mode
    global IOU_TH
    return {
        # "IOU_CONSTRAINT": 0.5,
        "IOU_CONSTRAINT": IOU_TH,
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
                    ...
                    for detNum in range(len(detPols)):
                        if (
                            ...
                            if (
                                ...
                                if evaluationParams["NED"]:
                                    ned = 1 - edit_distance(
                                        detTrans[detNum], gtTrans[gtNum]
                                    ) / max(len(detTrans[detNum]), len(gtTrans[gtNum]))
                                    detCorrect += ned
                                else:
    ...
    # for_copy_E2E_result = r"E2E_RESULTS: precision,recall,hmean: {},{},{}".format(
    #     methodPrecision, methodRecall, methodHmean
    # )
    # for_copy_det_only_result = r"DETECTION_ONLY_RESULTS: precision,recall,hmean: {},{},{}".format(
    #     det_only_methodPrecision, det_only_methodRecall, det_only_methodHmean
    # )
    for_copy_E2E_result = "E2E_PRH:{:.1f},{:.1f},{:.1f}".format(
        methodPrecision * 100, methodRecall * 100, methodHmean * 100
    )
    for_copy_det_only_result = "DET_PRH:{:.1f},{:.1f},{:.1f}".format(
        det_only_methodPrecision * 100,
        det_only_methodRecall * 100,
        det_only_methodHmean * 100,
    )
    ...
    resDict = {
        ...
        "for_copy_E2E_result": for_copy_E2E_result,
        "for_copy_det_only_result": for_copy_det_only_result,
    }
...
def text_eval_main(
    det_file, gt_file, is_word_spotting, IOU_threshold=0.5, is_NED=False
):
    global WORD_SPOTTING
